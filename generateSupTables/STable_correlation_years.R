...
for (y in 1:3){
  ...
  rownames(traitHSR1) <- traitHSR1$Accession
  rownames(traitHSR2) <- traitHSR2$Accession
  
  acc1 <- traitHSR1$Accession[!is.na(traitHSR1$N)]
  acc2 <- traitHSR1$Accession[!is.na(traitHSR2$N)]
  
  acc <- intersect(acc1,acc2)
  
  
  acc <- setdiff(acc2,acc1)
  hist(traitHSR2[acc,"N"])
  
  
  plot(traitHSR1[acc,"N"],traitHSR2[acc,"N"])
