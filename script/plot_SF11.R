...
genotypes_lab<-c("Seen genotypes","Unseen genotypes")
        ...
        for (r in 1:3){
          for (g in 1:2){
            ...
            if (file.exists(filen)){
              ...
              df=data.frame(Correlation=dat$value^2,Variable=newvar,Trait_type=trait_types[f],Genotype=genotypes_lab[g],Training_years=paste0(yearsp[r]))
...
ggplot(dfscore, aes(x=Variable, y=Correlation,fill=Training_years)) +
  ...
  labs(y="Correlation²",x="",fill="Training season") +
...
ggsave("figures/SF11.png",width = 13, height = 8)
