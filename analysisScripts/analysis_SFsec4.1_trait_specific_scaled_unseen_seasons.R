...
for (y in 1:6){
  for (i in 1:length(vars)){ #
    ...
    if (MLs[i]=="PLSR"){
      ...
      model.mse <- plsr(as.formula("y_trans~Spectra"),data=datasets[[1]], ncomp = noc.rcv.mse, method = plsmethod, scale = FALSE,validation = "none")
    }else if (MLs[i]=="SVR"){
      model.svr <- train(as.formula("y_trans~Spectra"),data = datasets[[1]],method="svmLinear", preProcess = c("center", "scale"),
    ...
    for (t in 1:2){
      ...
      mu <- mean(datasets[[1]]$y)
      sigma <- sd(datasets[[1]]$y)
      pred <- pred * sigma + mu
...
for (y in 1:3){
  for (i in 2:length(vars)){ #length(vars)
    inVar <- vars[i]
    datatype <- dts[dtsi[i]]
    ...
    if (MLs[i]=="PLSR"){
      iloop.rcv <- innerloop_repeatedCV(dataset=datasets[[1]],maxComps =maxnoc,method=plsmethod,iterations=repl,segments=segment,parallel=FALSE)
    ...
    for (t in 1:2){
      ...
      mu <- mean(datasets[[t]]$y)
      sigma <- sd(datasets[[t]]$y)
      pred <- pred * sigma + mu
