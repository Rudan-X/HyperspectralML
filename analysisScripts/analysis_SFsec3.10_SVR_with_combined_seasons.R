rm(list = ls())
list.of.packages <- c("pls","dplyr","plotrix","ggplot2","gridExtra","chemometrics","reshape2","caret")
...
for (y in 1:1){
  for (datatype in c("plot_averaged")){ #,"raw_data","genotype_averaged"
    for (inVar in vars){
      ...
      for (r in 1:nrep){
        ...
        for (f in 1:length(folds[[r]])){
          ...
          model.svr <- train(as.formula("y~Spectra"),data = datasets[[1]],method="svmLinear", preProcess=NULL,
      ...
      save(res.rcv.svr, file=paste0("results/compare_season/SVR_model_",datatype,"_",agg,year1[y],"and",year2[y],"_o", segments0*nrep,"_",inVar,".RData"))
    }
  }
}
...
for (y in 1:1){
  for (datatype in c("plot_averaged")){ #,"raw_data","genotype_averaged"
    for (inVar in vars){
      ...
      res.rcv.svr$folds <- folds
      save(res.rcv.svr, file=paste0("results/compare_season/SVR_model_",datatype,"_allyears_o", segments0*nrep,"i",segment*repl,"_",inVar,".RData"))
