...
for (y in 1:6){
  for (inVar in vars){
    ...
    for (d in 1:endd){ 
      ...
      save(val.ind,res.rcv.mse,file= paste0("results/unseen_seasons/PLSR_model_",datatype,"_",agg,"_training",year1[y],"_testing",year2[y],"_i",segment*repl,"_",inVar,".RData"))
    }
  }
}
...
for (y in 1:3){
  for (inVar in vars){
    ...
    for (d in 1:endd){ 
      ...
      res.rcv.mse$meas <- traitHSR$y
      save(val.ind,res.rcv.mse,file= paste0("results/unseen_seasons/PLSR_model_",datatype,"_",agg,"_training",unique(traitHSR1$Season),"_testing",year2[y],"_i",segment*repl,"_",inVar,".RData"))
    }
  }
}
