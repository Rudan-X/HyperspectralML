...
for (year in c(2021,2022,2023)){
  for (i in 1:1){ #length(vars)
    ...
    if (MLs[i]=="PLSR"){
      res.plsr <- list(R2 = templ, cor2=templ, RMSEP = templ, RMSEPper = templ, bestnoc = temp, pred = temp2)
    }else if (MLs[i]=="SVR"){
      
      res.svr <- list(R2 = templ, cor2=templ, RMSEP = templ, RMSEPper = templ, pred = temp2)
    }
    ...
    for (r in 1:nrep){
      ...
      for (f in 1:length(folds[[r]])){
        ...
        if (MLs[i]=="PLSR"){
          ...
          res.plsr$bestnoc[f,r] <- nComps.mse$bestnoc
          noc.mse <- nComps.mse$bestnoc
          
          model.smse <- plsr(as.formula("y~Spectra"),data=cal.data, ncomp = noc.mse, method = plsmethod, scale = FALSE,validation = "none")
          
          for (t in 1:2){
            
            pred.smse<-predict(model.smse, newdata=datasets[[t]], ncomp = noc.mse)
            if (t==1){
            ...
            res.plsr$R2[[t]][f,r] <- round(pls::R2(model.smse,newdata=datasets[[t]],intercept=F)[[1]][noc.mse],2)
            res.plsr$cor2[[t]][f,r] <- cor(pred.smse, datasets[[t]]$y)^2
          ...
          for (t in 1:2){
            ...
            res.svr$cor2[[t]][f,r] <- cor(pred.svr, datasets[[t]]$y)^2
            rmsep <- round(sqrt(mean(sq_resid)),2)
    ...
    if (MLs[i]=="PLSR"){
      ...
      save(res.plsr, file=paste0("results/unseen_genotypes_specific/PLSR_repeatedCV_model_",datatype,"_",agg,year,"_o", segments0*nrep,"i",segment*repl,"_",inVar,".RData"))
