...
innerloop_repeatedCV <- function(dataset=NULL, formula= as.formula("y~Spectra"),maxComps=20, method = "oscorespls", iterations=30, segments=3,parallel=FALSE) {
  ...
  for (i in 1:iterations) {
    ...
    if (parallel){
      ...
      res <- plsr(formula, data=dataset, ncomp = maxComps,  
