...
find_optimal_components <- function(dataset=NULL, method="pls", maxComps=20, iterations=20, seg=100, 
  ...
  if(method=="pls") {
    ...
    plsr.out <- pls::plsr(as.formula("y~Spectra"), scale=FALSE, center=TRUE, ncomp=maxComps, 
  ...
  if(method=="firstPlateau") {
    press.out <- pls_permutation(dataset=dataset, maxComps=maxComps, 
