  ...
  for (d in 1:length(datatype)){#
    for (inVar in vars){
      ...
      if (file.exists(filen)){
        ...
        for (t in 1:2){
          if (ini){
            dfr2<-data.frame(score=apply(res.rcv.svr$R2[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year)
            dfrmse<-data.frame(score=apply(res.rcv.svr$RMSEP[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year)
            dfrmseper<-data.frame(score=apply(res.rcv.svr$RMSEPper[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year)
            dfnoc<-data.frame(NoC=apply(res.rcv.svr$bestnoc, 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year)
            ini<-FALSE
          }else{
            dfr2<-rbind(dfr2,data.frame(score=apply(res.rcv.svr$R2[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year))
            dfrmse<-rbind(dfrmse,data.frame(score=apply(res.rcv.svr$RMSEP[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year))
            dfrmseper<-rbind(dfrmseper,data.frame(score=apply(res.rcv.svr$RMSEPper[[t]], 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year))
            dfnoc<-rbind(dfnoc,data.frame(NoC=apply(res.rcv.svr$bestnoc, 2, mean),trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d], Season=year))
          }
...
ggplot(dfr2[dfr2$data_split=="Validation",], aes(x=trait, y=score,fill=datatype)) +
  geom_boxplot()+
  facet_grid(Season ~ Trait_type,scales="free")+ #, switch = "y"
...
ggsave("Figures/SF_sec3.7_SVR_aggregation_season.png",width = 10, height = 9)
