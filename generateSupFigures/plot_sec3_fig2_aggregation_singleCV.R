...
for (d in 1:3){
  ...
  for (inVar in vars){
    ...
    t <- 2
    if (ini){
      dfr2<-data.frame(score=melt(res.singlecv.mse$R2[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d])
      dfrmse<-data.frame(score=melt(res.singlecv.mse$RMSEP[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d])
      dfrmseper<-data.frame(score=melt(res.singlecv.mse$RMSEPper[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d])
      dfnoc<-data.frame(NoC=melt(res.singlecv.mse$bestnoc)$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d])


      ini<-FALSE
    }else{
      dfr2<-rbind(dfr2,data.frame(score=melt(res.singlecv.mse$R2[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d]))
      dfrmse<-rbind(dfrmse,data.frame(score=melt(res.singlecv.mse$RMSEP[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d]))
      dfrmseper<-rbind(dfrmseper,data.frame(score=melt(res.singlecv.mse$RMSEPper[[t]])$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d]))
      dfnoc<-rbind(dfnoc,data.frame(NoC=melt(res.singlecv.mse$bestnoc)$value,trait=inVar,Selection="RepeatedCV",Aggregation=dtlabel[d]))
    }
...
ggsave("Figures/SF_sec3.2_aggregation_singleCV.png",width = 10, height = 12)
