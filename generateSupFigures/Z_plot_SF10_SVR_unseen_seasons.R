...
for (y in 1:3){
  for (datatype in c("plot_averaged")){ 
    for (inVar in vars){
      ...
      load(file=paste0("results/unseen_seasons/SVR_model_",datatype,"_",agg,"_training",year1[[y]][1],"&",year1[[y]][2],"_testing",year2[y],"_",inVar,".RData"))
...
ggplot(dfcor, aes(x=Measured, y=Predicted,color=Training)) +
  ...
  stat_cor(aes(label = after_stat(rr.label)),label.x.npc = 0.75,label.y.npc = 0.48,size=3)+
...
ggsave("Figures/SF10_predicting_unseen_seasons.png",width = 7, height = 11)
