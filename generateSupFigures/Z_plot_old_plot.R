...
for (d in 2:2){#
  ...
  for (inVar in vars){
    ...
    for (r in 1:1){
      ...
      if (ini){
        df<-data.frame(Predicted=pred,Measured=traitHSR$y,trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d])
        ini <- FALSE
      }else{
        df<- rbind(df,data.frame(Predicted=pred,Measured=traitHSR$y,trait=inVar,data_split=dataset[t],Model="PLSR",datatype=datalabel[d]))
      }
...
rmse_df <- df %>%
  ...
  mutate(
    ...
    y_nudge = case_when(
              datatype == "Raw data" ~ 0.4,
              datatype == "Averaged by replicate" ~ 0.3,
              datatype == "Averaged by genotype" ~ 0.2),
  ) %>% 
  mutate(
...
ggsave("Figures/SF_sec4.2_PLSR_unseen_seasons_PLOT_scatterplot.png",width = 7, height = 14)
