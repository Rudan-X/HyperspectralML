...
for (y in 1:6){
  for (d in 1:3){ 
    ...
    for (inVar in vars){
      ...
      filen <- paste0("results/unseen_seasons/SVR_model_",datatype,"_",agg,"_training",year1[y],"_testing",year2[y],"_",inVar,".RData")
...
for (y in 1:3){
  for (d in 1:3){ 
    datatype <- datatypes[d] 
    for (inVar in vars){
      filen <- paste0("results/unseen_seasons/SVR_model_",datatype,"_",agg,"_training",year1[[y]][1],"&",year1[[y]][2],"_testing",year2[y],"_",inVar,".RData")
      if (file.exists(filen)){
        load(filen)
        
        dfcor <- rbind(dfcor,data.frame(score=cor(res.rcv.svr$pred[val.ind],res.rcv.svr$meas[val.ind])^2,Trait=inVar,Training="Combined",Testing=year2[y],datatype=datalabel[d]))
      }
    }
  }
}
...
ggsave("Figures/SF_sec4.1_SVR_predicting_unseen_seasons_barplot.png",width = 12, height = 8)
