  ...
  for (y in 1:3){
    ...
    for (f in 1:3){
      for (varname in vars[[f]]){
        for (r in 1:3){
          filen=paste0("results/scenario3/maize_",methods[m],"_", varname,"_testY",testing_year,"_trainY",yearsp[r],"_",aggtype,".RData")
          if (file.exists(filen)){
            load(filen)
            dat=melt(testing_corr)
            newvar<-var2[match(varname,var1)]
            df=data.frame(Correlation=dat$value,Variable=newvar,Trait_type=trait_types[f],Method=methods[m])
            df$Genotype<-"Seen genotypes"
            df$Genotype[dat$Var3==2]="Unseen genotypes"
            df$Training_years<-paste0(yearsp[r])
            df$Testing_year<-paste0(yearsused[y])
...
dfscore$Training_years<-factor(dfscore$Training_years, levels=c("2021","2022","2023","2021&2022","2023&2021","2022&2023"))
...
g1<-ggplot(dfscore[dfscore$Variable=="Vpmax",], aes(x=Training_years, y=Correlation,fill=Method)) +
  geom_boxplot()+
  facet_grid(Genotype ~ Testing_year,scales="free", switch = "y")+
  scale_y_continuous(position = "right" ,limits = c(-1, 0.9) )+ #
  labs(y="",x="") +
  stat_summary(fun.data = stat_box_data,geom = "text", aes(group=Method), size=4,position=position_dodge(0.9)) +
...
ggsave("figures/MF4C.png",width = 16, height = 10)
