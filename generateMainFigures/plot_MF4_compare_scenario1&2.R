...
for (year in c(2021,2022,2023)){
  for (i in 1:length(vars)){
    ...
    if (ini){
      dfr2<-data.frame(score=melt(res$R2[[2]])$value,trait=inVar,AggregationML=paste0(dtlabels[dtsi[i]], " & ", MLs[i]), Season=year, Scenario = "Unseen genotypes")
      ini<-FALSE
    }else{
      dfr2<-rbind(dfr2,data.frame(score=melt(res$R2[[2]])$value,trait=inVar,AggregationML=paste0(dtlabels[dtsi[i]], " & ", MLs[i]), Season=year, Scenario = "Unseen genotypes"))
    }
  }
}
...
paired_colors <- c(
  "Random split" = "#7FBF7F",    
  "Unseen genotypes" = "#C2A5CF"

)

paired_colors_label <- c(
  "Random split" = "#1B7837",       # darker green
  "Unseen genotypes" = "#762A83"    # darker purple
)
...
ggplot(dfr2, aes(x = trait, y = score, fill = Scenario)) +
  ...
  stat_compare_means(label = "p.signif",label.y = -0.7) 


ggsave("Figures/MF4_compare_scenario1&2.png",width = 8, height = 10)
